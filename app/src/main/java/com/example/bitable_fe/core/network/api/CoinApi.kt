...
interface CoinApi {
    ...
    @POST("/api/coins")
    suspend fun registerCoin(
        @Body request: RegisterCoinRequest
    ...
    @POST("/api/coins/initialize")
    suspend fun initializeCoins(): ApiResponse<Void?> // 서버 응답에 따라 조정
}
